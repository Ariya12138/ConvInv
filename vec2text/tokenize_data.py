...
from vec2text.models.model_utils import BGE
...
def embed_dataset_batch(model: InversionModel, batch: Dict) -> Dict:
    ...
    with torch.no_grad():
        if isinstance(model.embedder, BGE):
            input_ids = emb_input_ids["input_ids"].to(next(model.parameters()).device)
            attention_mask = emb_input_ids["attention_mask"].to(next(model.parameters()).device)
            batch["frozen_embeddings"] = model.call_embedding_model(input_ids, attention_mask)
        else:
            emb_input_ids = emb_input_ids.to(next(model.parameters()).device)
