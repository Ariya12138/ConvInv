...
class Corrector(BaseTrainer):
    ...
    def _generate_with_beam(
        ...
        frozen_embeddings = inputs["frozen_embeddings"]
        #initial_hypothesis_str = inputs["initial_hypothesis_str"]
        #self.initial_hypothesis_str = initial_hypothesis_str
        ################################################################################
        ...
        if (num_recursive_steps_so_far == 0) and (
            ...
            bt_gen_text_ids = []
            for hypothesis_str in self.initial_hypothesis_str:
                gen_text_ids = (
                    ...
                    .repeat((sequence_beam_width, 1))
                    .to(self.args.device)
                )
                
                bt_gen_text_ids.append(gen_text_ids)
            gen_text_ids = torch.cat(bt_gen_text_ids, dim=0)
            ...
            bos_token_ids = (
                torch.ones(
                    (sequence_beam_width*batch_size, 1), dtype=torch.long, device=gen_text_ids.device
                )
                * bos_token_id
            )
            #print(bos_token_ids.shape)
            gen_text_ids = torch.cat((bos_token_ids, gen_text_ids[:, :-1]), dim=1)
            #print(gen_text_ids.shape)
        else:
            ...
            #print("gen_text_ids:", gen_text_ids)
        ...
        if gen_text_ids.shape[0] > batch_size:
            if sequence_beam_width == 1:
                ...
                if self.initial_hypothesis_str is not None and num_recursive_steps_so_far == 0:
                    best_idx_in_beam = best_idx_in_beam_total
